...
import {
  LayoutDashboard, Globe, Bell, FileText, Settings,
  LogOut, ChevronRight, AlertTriangle, Shield
} from 'lucide-react'
...
export default function AppLayout() {
  ...
  async function loadCounts() {
    const { data } = await supabase
      .from('routes')
      .select('id, risk_analyses(risk_score)')
      .eq('user_id', user.id)
    ...
    if (data) {
      ...
      const pending = data.filter(r => !r.risk_analyses?.length).length
