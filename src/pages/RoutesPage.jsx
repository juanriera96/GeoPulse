...
import { Globe, Plus, Trash2, Edit2, X, Package, TrendingUp, MapPin, Zap, Loader2, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function RoutesPage() {
  ...
  async function handleAnalyze(route) {
    ...
    try {
      ...
      toast.success('Analisis completado. Score: ' + result.risk_score)
      fetchRoutes()
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {routes.length === 0 ? (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">Sin rutas todavia</h3>
            ...
            return (
              <div key={route.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-slate-700 transition-colors group">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">{route.cargo_type}</p>
                    </div>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  {route.description && (
                    <p className="text-slate-500 text-xs mt-1 line-clamp-2">{route.description}</p>
                  )}
                </div>
                {isExpanded && route.risk_analyses && route.risk_analyses.length > 0 && (
                  <div className="mb-4 p-3 bg-slate-800 rounded-lg border border-slate-700">
                    <p className="text-xs text-slate-400">
                      Ultimo analisis: {new Date(route.risk_analyses[0].created_at).toLocaleDateString('es', {
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isAnalyzing ? (
                      ...
                      <><Zap className="w-3.5 h-3.5" /><span>Analizar</span></>
                    )}
                  </button>
